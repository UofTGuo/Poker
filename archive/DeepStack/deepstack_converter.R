...
phil.l <- read.csv("ds_phil_l.csv",stringsAsFactors=F)
dmitry.l <- read.csv("ds_dmitry_l.csv",stringsAsFactors=F)
antonio.p <- read.csv("ds_antonio_p.csv",stringsAsFactors=F)
##
...
win_prob = function(numattable1=2,dealt_index, round, iters=100){
  all_index <- sample(52)
...
##===Equity Calculator===
equity <- function(pf.log,f.log,t.log,r.log,dealt_index,DSbb,numattable1=2, blinds1=c(50,100),iters=100){
  ### Pre-flop equity
...
time1 <- Sys.time()
for(i in 1:nrow(test)){
	cat( equity(pf.log[[i]],f.log[[i]],t.log[[i]],r.log[[i]],cards[[i]],DSbb[i],iters=1000),"\n" )
}
time2 <- Sys.time()
time2-time1
##
...
#colMeans(dat) 	
#sqrt(diag(var(dat[,1:6]))/nrow(dat))

DSbb.dat <- dat[dat$V7==1,]
mbb <- colMeans(DSbb.dat[,1:6])
sebb <- sqrt(diag(var(DSbb.dat[,1:6])))/sqrt(nrow(DSbb.dat))
corrbb <- cor(DSbb.dat[-7])[1,3]
slrbb <- mbb[3]/mbb[1]
pffbb <- length(DSbb.dat$V5[DSbb.dat$V5==50])/nrow(DSbb.dat)

DSsb.dat <- dat[dat$V7==0,c(2,1,4,3,6,5,7)]
names(DSsb.dat) <- c("V1","V2","V3","V4","V5","V6","V7")
msb <- colMeans(DSsb.dat[,1:6])
sesb <- sqrt(diag(var(DSsb.dat[,1:6])))/sqrt(nrow(DSsb.dat))
corrsb <- cor(DSsb.dat[-7])[1,3]
slrsb <- msb[3]/msb[1]
pffsb <- length(DSsb.dat$V5[DSsb.dat$V5==-50])/nrow(DSsb.dat)

DS.dat <- rbind(DSbb.dat,DSsb.dat)
mean1 <- colMeans(DS.dat[,1:6])
se1 <- sqrt(diag(var(DS.dat[,1:6])))/sqrt(nrow(DS.dat))
corr1 <- cor(DS.dat[-7])[1,3]
slr1 <- mean1[3]/mean1[1]
pff1 <- (length(DSbb.dat$V5[DSbb.dat$V5==50])+length(DSsb.dat$V5[DSsb.dat$V5==-50]))/nrow(DS.dat)

round(mbb,3); round(sebb,2); round(c(corrbb,slrbb,pffbb),3)
round(msb,3); round(sesb,2); round(c(corrsb,slrsb,pffsb),3)
round(mean1,3); round(se1,2); round(c(corr1,slr1,pff1),3)
##

##==Martin S==
dat <- read.table("martin_s.txt")
#colMeans(dat) 	
#sqrt(diag(var(dat[,1:6]))/nrow(dat))
